...
uniform_real_distribution<long double> distribution2(0.0, 1.0);
...
struct tree {
    ...
    vector<long double> norms; // vector containing the denominator of the density
    ...
    tree(int a, Vector4f z, int depth, float** coeffcarre, float** coeff) {
        ...
        value_function = vector<long double>(2 * power, -INFINITY);
        ...
        norm = normalisation(coeffcarre, riemann_number, d, sigma_theta);
        if (isinf(norm)) {
            cout << "ERREUR";
            norm = numeric_limits<long double>::max();
        }
           
        if (norm == 0) {
            norm = numeric_limits<long double>::min();
            cout << "fuck";
        }
	...
	Vector4f temp(0,0,0,0);
        position = z + temp;
        densities = vector<vector<float>>(2 * d + 1, vector<float>(power * (2 * d + 1), 0));//2*d+1 rows matrix with n_theta*2 columns (n_theta 2d+1*2d+1 matrices)
        value_function = vector<long double>(2 * power, -10);
        probaMatrix = vector<float>(power * n_A, 1 / (float)n_A);
        ...
        for (int teta = 0; teta < n_theta; teta++) { //for every new theta we compute the bernstein polynomial
            ...
            //cout << polyValue(coeffcarre,d, Vector4f(-1, 0, 0.5, 0)) << " Value coef " << Fun(discretizedThetas[teta],-1,0.5,z,sigma_theta) <<  endl;
            for (int prev_index = 0; prev_index < pow(n_theta, H - depth-1); prev_index++) { //for every previous density we add the new bernstein approximation to the previous densities
                ...
                for (int i = 0; i < 2 * d + 1; i++) {
                    for (int j = 0; j < 2 * d + 1; j++) {
                        densities[i][j + teta * (2 * d + 1) + prev_index * n_theta * (2 * d + 1)] = coeffcarre[i][j] + prev->densities[i][j + prev_index * (2 * d + 1)]; //+ l'ancienne densit�
                    }
                }
            }
        }
        for (int l = 0; l < densities[0].size() / (2 * d + 1); l++) {
            norm = normalization(densities, riemann_number, d, l,sigma_theta);
            if (isinf(norm)) {
                cout << "ERREUR";
                norm = numeric_limits<long double>::max();
            }
...
void bellman(tree* t, int n) { //n is the depth 
    ...
    long double best_action = 0;
    int indexNextDensity;
    vector<long double> value_per_action = vector<long double>(n_A, 0);
...
//Routine cr�ant l'arbre � partie de la distribution initiale 
...
float path_value(policy* p, tree* t, float theta) {
    ...
    for (int h = 1; h < H; h++) {
        t = t->childs[action];
        theta_rand = atan2(t->position[2],t->position[0])  +normal(generator);
        p = p->childs[action];
...
long double phiproba(tree *t, policy *p, int h) {
    if (h == H) {
        return 1;
    }
    long double value = 1;
...
long double log_phiproba(tree* t, policy* p, int h) {
    if (h == H) {
        return 0;
    }
    long double value = 0;
...
vector<int> ASA(tree* t, int Nk, int Mk, float theta,int* index) {
    ...
    long double phi_denominator;
    float test_sum = 0;
    int k_max = 20;
    long double logphi;
    long double max_an = 0;
    long double max_bn = 0;
...
int main(int argc, char** argv)
{
    ...
    srand(time(NULL));
    uniform_real_distribution<long double> distrib_angle(0.0, PI);
    float angle_z = distrib_angle(generator);
    float dist_z = 12;
    float v_z = 1.5;
    float vtarget = 0;
    ...
    Vector4f z(dist_z*cos(angle_z), -v_z*sin(angle_z), dist_z*sin(angle_z), v_z*cos(angle_z));
    //Vector4f z(-7,0,0,2);
    //Theta history
    vector<float> thetas;
    ...
    file_p << path.back()[0] << "," << path.back()[2] << "," << 0<< "," << 0 << "," << 0 << endl;
    ...
    file_v.open("valuefunction_file.txt", ios_base::out | ios_base::in);
        ...
        for (int n = 0; n < N; n++) {
            ...
            for (int h = 0; h < H + 1; h++) {
                ...
                thetas.push_back(atan2(path.back()[2]-x[2], path.back()[0]-x[0]) + distribution(generator));
                ...
                file_p << path.back()[0] << "," << path.back()[2] << "," << discretizedThetas[index_theta] << "," << thetas.back() << "," << temp << endl;
                for (int i = 0; i < 2 * d + 1; i++) {
                    for (int j = 0; j < 2 * d + 1; j++) {
    ...
    for (int n = 0; n < N; n++) {
        ...
        if (n == 0) {
            ...
            t = new tree(0, z, dummy, index_value, H, coeffcarre, coeff);
        ...
        for (int h = 0; h < H + 1; h++) {
            float temp = distribution(generator);
            thetas.push_back(atan2(path.back()[2] - x[2], path.back()[0] - x[0]));
            index_theta = nearest_theta(thetas[n * (H + 1) + h]);
            if (h == 0) {
                index_value = 0;
            }
